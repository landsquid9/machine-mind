...
Mind::Plant::~Plant()
{
	Ogre::LogManager::getSingletonPtr()->logMessage("deconstructing plant");
}

bool Mind::Plant::setup(Ogre::SceneManager * sm, Ogre::Real x, Ogre::Real z, Ogre::Real sc)
{
	sceneManager = sm;
	...
	raycastFromPoint(Ogre::Vector3(x, 1000, z), Ogre::Vector3(0, -1, 0), closestPoint);

	posX = x;
	posY = closestPoint.y;
	posZ = z;

	plantEntity = sm->createEntity("plant_01.mesh");

	plantNode   = sm->getRootSceneNode()->createChildSceneNode();

	plantNode->attachObject(plantEntity);
	plantNode->setPosition(posX, posY, posZ);
	scale = sc;
...
void Mind::Plant::update(Ogre::Real timeInterval)
{
	if (scale > 0)
	{
		scale -= timeInterval * scaleSpeed;
		plantNode->setScale(scale, scale, scale);
	}
	
}

void Mind::Plant::remove()
{
	sceneManager->destroyEntity(plantEntity);
	Ogre::LogManager::getSingletonPtr()->logMessage("destroyed children");
}
...
bool Mind::Plant::raycastFromPoint(const Ogre::Vector3 &point,
...
void Mind::Plant::getMeshInformation(const Ogre::MeshPtr mesh,
	size_t & vertex_count,
	Ogre::Vector3 *& vertices,
	size_t & index_count,
	unsigned long *& indices,
